...
filtered<-fread(args[1], header=T, stringsAsFactors = F, verbose=T, select=c("SNP", "F_MISS"), key = c("SNP"))
combined<-fread(args[2], header=T, stringsAsFactors = F, verbose=T, select=c("SNP", "F_MISS"), key = c("SNP"))


df<-merge(filtered,combined, by="SNP", all.y=T)
df_<-melt(df, id.vars = "SNP")
p<-ggplot(df_, aes(value, fill=variable)) + geom_density(alpha=0.2)+ggtitle("Fraction_missing_per_variant")+ylab("Density")+labs(fill="")+scale_fill_discrete(labels=c("Filtered", "Combined"))
